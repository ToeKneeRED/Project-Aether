...
void FEZSplitModule::StartupModule()
{
    ...
    if (UToolMenus::IsToolMenuUIEnabled())
    {
        ...
        Section.AddMenuEntry(
            ...
            FText::FromString("Split By Loose Parts"),
            FText::FromString("Splits the selected static mesh actor into separate parts by poly group."),
            FSlateIcon(),
            FUIAction(FExecuteAction::CreateRaw(this, &FEZSplitModule::OnSplitLoosePartsButtonClicked))
        );

        Section.AddMenuEntry(
            "Split By Material",
            FText::FromString("Split By Material"),
            FText::FromString("Splits the selected static mesh actor into separate parts by material."),
            FSlateIcon(),
            FUIAction(FExecuteAction::CreateRaw(this, &FEZSplitModule::OnSplitMaterialButtonClicked))
        );
...
void FEZSplitModule::AddMenuEntry(FMenuBuilder& MenuBuilder)
{
    ...
    {
        UE_LOG(LogEZSplit, Log, TEXT("EZSplit: Adding Split Loose Parts menu entry for selected static mesh actor."));
        ...
        UE_LOG(LogEZSplit, Log, TEXT("EZSplit: Adding Split Material menu entry for selected static mesh actor."));
        MenuBuilder.AddMenuEntry(
            FText::FromString("Split By Material"),
            FText::FromString("Splits the selected static mesh into separate parts by material."),
            FSlateIcon(),
            FUIAction(FExecuteAction::CreateRaw(this, &FEZSplitModule::OnSplitMaterialButtonClicked))
        );
...
void FEZSplitModule::OnSplitLoosePartsButtonClicked()
{
    ...
    {
        SplitLooseParts(Actor);
    }
}

void FEZSplitModule::OnSplitMaterialButtonClicked()
{
    // Access the selected actor
    TArray<AActor*> SelectedActors;
    if (GEditor && GEditor->GetSelectedActors())
...
// Split
void SplitLooseParts(AActor* Actor)
{
    UStaticMesh* BaseMesh;

    // Grab actor transform
    ...
    TArray<UStaticMesh*> SplitMesh = CreateNewStaticMeshes(BaseMesh, Vertices, TriangleIndices, Components, Normals, Tangents, Colors, UVChannels, EzSplitType::LooseParts);

    HandleSplitMeshPlacement(Actor, ActorTransform, SplitMesh, BaseMesh->GetName());
}

void SplitMaterial(AActor* Actor)
{
    UStaticMesh* BaseMesh;
    ...
    // Find materials/sections
    TArray<TArray<uint32>> Components;
    FindMaterials(BaseMesh, Vertices, TriangleIndices, Components);

    // Create new static meshes for each component
    TArray<UStaticMesh*> SplitMesh = CreateNewStaticMeshes(BaseMesh, Vertices, TriangleIndices, Components, Normals, Tangents, Colors, UVChannels, EzSplitType::Material);
    ...
    TArray<TArray<uint32>>& OutComponents)
{
    ...
    // One component per section/material
    OutComponents.Empty();
    OutComponents.SetNum(LODResource.Sections.Num());
    ...
    for (int32 SectionIndex = 0; SectionIndex < LODResource.Sections.Num(); ++SectionIndex)
    {
        ...
        // The FirstIndex is into the INDEX buffer (in indices), so /3 to get the triangle index
        int32 FirstTriangle = Section.FirstIndex / 3;
        int32 NumTris       = Section.NumTriangles;

        // For each triangle in this section, push its triangle‐index
        for (int32 TriOffset = 0; TriOffset < NumTris; ++TriOffset)
        {
            OutComponents[SectionIndex].Add(FirstTriangle + TriOffset);
        }
    }
}
    ...
    const EzSplitType SplitType)
{
    ...
    for (int32 ComponentIndex = 0; ComponentIndex < Components.Num(); ++ComponentIndex)
    {
        ...
        if (NewStaticMesh)
        {
            ...
            while (MeshDescription->PolygonGroups().Num() < BaseMesh->GetStaticMaterials().Num())
            {
            ...
            if (SplitType == Material)
            {
                // 1) Grab the *existing* material list, but make a local copy we can prune
                TArray<FStaticMaterial> PrunedMats = NewStaticMesh->GetStaticMaterials();

                // 2) Walk it backwards and drop any slots that never got used
                for (int32 MatIdx = PrunedMats.Num() - 1; MatIdx >= 0; --MatIdx)
                {
                    bool bUsed = false;
                    const FMeshDescription* Desc = NewStaticMesh->GetMeshDescription(0);  // or loop LODs if you want
                    if (Desc)
                    {
                        for (const FPolygonID& PolyID : Desc->Polygons().GetElementIDs())
                        {
                            if (Desc->GetPolygonPolygonGroup(PolyID).GetValue() == MatIdx)
                            {
                                bUsed = true;
                                break;
                            }
                    ...
                    if (!bUsed)
                    {
                        PrunedMats.RemoveAt(MatIdx);
                    }
                }

                // make sure the mesh only has one slot, the one you split out:
                UMaterialInterface* SectionMat =
                    BaseMesh->GetStaticMaterials()[ComponentIndex].MaterialInterface;

                TArray<FStaticMaterial> OneMat;
                OneMat.Add( FStaticMaterial(SectionMat) );

                NewStaticMesh->Modify();                // for undo/redo
                NewStaticMesh->SetStaticMaterials(OneMat);
                NewStaticMesh->GetSectionInfoMap().Clear();  // reset section→slot mapping
            }
    ...
    const FString& BaseMeshName)
{
    ...
    for (UStaticMesh* SplitMesh : SplitMeshes)
    {
        ...
        AStaticMeshActor* NewActor = World->SpawnActor<AStaticMeshActor>(AStaticMeshActor::StaticClass(), ActorTransform);
